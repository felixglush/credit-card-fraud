...
def card_precision_top_k_custom(y_true, y_pred, top_k, transactions_df):
    ...
    nb_frauded_cards_per_day, card_precision_top_k_per_day_list, mean_card_precision_top_k = card_precision_top_k(
...
def get_tx_stats(transactions_df, start_date_df="2018-04-01"):
    ...
    nb_frauded_card_per_day = transactions_df[transactions_df['TX_FRAUD'] == 1].groupby(
...
def get_performance_plot(performances_df,
    ...
    for i in range(len(expe_type_list)):
        ...
        if performance_metric_expe_type + ' Std' in performances_df.columns:
            conf_min = performances_df[performance_metric_expe_type] - 2 * performances_df[
                performance_metric_expe_type + ' Std']
            conf_max = performances_df[performance_metric_expe_type] + 2 * performances_df[
                performance_metric_expe_type + ' Std']

            ax.fill_between(performances_df['Parameters summary'], conf_min, conf_max, color=expe_type_color_list[i],
                            alpha=.1)
